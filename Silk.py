...
dataset=[]
label=[]
no_tumor=os.listdir(image_directory+'diseased/')
yes_tumor=os.listdir(image_directory+'undiseased/')
...
for i ,image_name in enumerate(no_tumor):
    if (image_name.split('.')[1]=='jpg'):
        image=cv2.imread(image_directory+'diseased/'+image_name)
...
for i ,image_name in enumerate(yes_tumor):
    if (image_name.split('.')[1]=='jpg'):
        image=cv2.imread(image_directory+'undiseased/'+image_name)
        image=Image.fromarray(image,'RGB')
        image=image.resize((64,64))
        dataset.append(np.array(image))
        label.append(1)
